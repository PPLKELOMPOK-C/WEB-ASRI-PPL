...
<div style="padding:48px 5%">
    <div class="container" style="padding:0;display:grid;grid-template-columns:1fr 300px;gap:36px;align-items:start">
        ...
        <div>
            ...
            <h1 style="font-family:'Playfair Display',serif;font-size:34px;font-weight:700;color:var(--green-900);line-height:1.3;margin-bottom:24px">{{ $article->judul }}</h1>
            ...
            <div style="font-size:15px;line-height:1.9;color:#2d3d2d;white-space:pre-wrap">{{ $article->konten }}</div>
        </div>
            ...
            <div style="background:white;border:1px solid #e8f0eb;border-radius:12px;padding:20px">
                ...
                <a href="{{ route('public.news.detail', $r->slug) }}" style="display:flex;gap:10px;text-decoration:none;padding:10px 0;border-bottom:1px solid #f0f5f1">
                    <div style="width:60px;height:50px;background:var(--green-100);border-radius:6px;flex-shrink:0;overflow:hidden">
                        ...
                        @else
                        <div style="display:flex;align-items:center;justify-content:center;height:100%"><i class="ri-newspaper-line" style="color:var(--green-400)"></i></div>
                        @endif
                    </div>
                    <div>
                        <div style="font-size:13px;font-weight:600;color:var(--green-900);line-height:1.3">{{ Str::limit($r->judul, 50) }}</div>
                        <div style="font-size:11px;color:#5a7a5a;margin-top:3px">{{ $r->published_at->format('d M Y') }}</div>
                    </div>
                </a>
                @endforeach
            </div>
